<template>
    <section
        ...
        <div class="flex flex-row justify-between">
            <legend class="w-1/3">
                <a :href="Object.values(yearSpreadsheetUrls).slice(-1)">XLSX</a>
                overwrite<br /><input
                    type="file"
                    name="file"
    ...
    <LoadingIndicator
        v-if="!latestFsrYear"
        class="h-8 w-8"></LoadingIndicator>
    <div v-else>
        <div
            v-if="selectedYear"
            class="prose dark:prose-invert mb-4 max-w-none border-b border-gray-300 pb-4 text-lg">
...
<script>
    ...
    const yearSpreadsheetUrls = import.meta.glob("./assets/years/*.xlsx", {
        query: "?url",
        import: "default",
        eager: true,
    });
    ...
    export default {
        ...
        mounted() {
            Promise.all(
                Object.values(this.yearSpreadsheetUrls).map(
                    (spreadsheetUrl) => {
                        return new Promise((resolve, reject) => {
                            fetch(spreadsheetUrl)
                                .then((response) => response.blob())
                                .then((blob) =>
                                    store().instanciateYearFromFile(blob),
                                )
                                .then(() => {
                                    resolve();
                                });

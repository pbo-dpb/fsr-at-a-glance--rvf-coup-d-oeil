<template>
    <figure class="@container/map relative mx-auto lg:w-2/3">
        <div
            class="h-fit w-full dark:opacity-50"
            ref="infographic"></div>
        <div
            ...
            class="flex scale-75 cursor-pointer flex-col items-center gap-1 p-1 shadow-xl ring-blue-800 ring-offset-2 transition-all hover:ring-2 @xl/map:scale-100"
            @mouseover="highlightRegion(region)"
            @mouseleave="highlightRegion(null)"
            @click="selectedRegion = region">
            <div
                class="overflow-hidden text-center text-lg font-semibold whitespace-nowrap text-blue-800">
                {{
                    region.id === "territories"
                        ? "Terr."
                        : region.id.toUpperCase()
                }}
            </div>
            <img
                v-if="region.is_sustainable"
                :src="sustainabilityGlyphTrue"
                class="h-6 w-6" />
            <img
                v-if="region.is_sustainable === false"
                :src="sustainabilityGlyphFalse"
                class="h-6 w-6" />
            <img
                v-if="region.is_sustainable === null"
                :src="sustainabilityGlyphNull"
                class="h-6 w-6" />
        </div>
    </figure>
</template>
    ...
    export default {
        ...
        methods: {
            applyHighlightToSvgDomEl(domEl) {
                if (!domEl) return;
                domEl
                    .querySelectorAll("path, polygon")
                    .forEach((s) => this.applyHighlightToSvgDomEl(s));
            ...
            drawRegionHighlight() {
                ...
                if (
                    ...
                    this.highlightedRegion === "territories"
                ) {
                    highlightedGs = [`#CA_YT`, `#CA_NT`, `#CA_NU`];
                } else if (this.highlightedRegion) {
                    highlightedGs = [
                        `#CA_${this.highlightedRegion.toUpperCase()}`,
                    ];
                }
                ...
                highlightedGs.forEach((highlightedG) => {
                    ...
                    if (regionG) this.applyHighlightToSvgDomEl(regionG);
                });
            },
            redraw() {

<template>
    <figure class="flex flex-col gap-4">
        <div class="flex flex-row justify-end">
            <label
                class="flex flex-row items-center gap-2 rounded-sm border border-solid border-purple-800 bg-purple-50 px-2 py-1 text-sm font-semibold text-purple-800 select-none">
                <input
                    type="checkbox"
                    :checked="compareWith"
                    @change="setCompareWith(null)" />
                {{ strings.indicators_comparison_toggle }}
            </label>
        </div>
        <RegionViewIndicatorsGroup
            :region="region"
            v-for="(indicators, group) in groups"
            :key="group"
            :group="group"
...
<script>
    ...
    const localStorageKey =
        "ca.pbo-dpb.pbo-dpb_fsr-at-a-glance--rvf-coup-d-oeil.displayComparisonTo";
    export default {
        props: {
            region: {
                type: Region,
        ...
        created() {
            ...
            if (val && val == "nat_avg") this.compareWith = this.selectedYear;
        },
        computed: {

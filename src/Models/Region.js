...
export default class Region {
    ...
    static buildRegionsFromRows(indicators, sustainability, bullets, charts) {
        ...
        return regionsIso.map((iso) => {
            let regionIndicators = Indicators.initRegionIndicatorsFromRows(
                iso,
                indicators,
            );
            let regionBullets = bullets
                .filter((row) => row[0] === iso)
                .map((row) => new Bullet({ en: row[1], fr: row[2] }));
            let sustainabilityRow = sustainability.find(
                (row) => row[0] === iso,
            );
            ...
            let region = new Region({
                ...
                sustainability_label:
                    sustainabilityRow[2] && sustainabilityRow[3]
                        ? { en: sustainabilityRow[2], fr: sustainabilityRow[3] }
                        : null,
                datasets: {
                    net_debt: Dataset.initRegionDatasetsFromRows(
                        iso,
                        charts,
                        "net_debt",
                    ),
                    revenue: Dataset.initRegionDatasetsFromRows(
                        iso,
                        charts,
                        "revenue",
                    ),
                    program_spending: Dataset.initRegionDatasetsFromRows(
                        iso,
                        charts,
                        "program_spending",
                    ),
